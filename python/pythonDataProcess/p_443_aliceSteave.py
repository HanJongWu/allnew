...
alias_color_mask = np.array((Image.open(alice_color_file)))

wc = WordCloud(background_color='white', max_words=2000, mask=alias_color_mask, stopwords=mystopwords, max_font_size=40, random_state=42)
wc = wc.generate(text)
...
plt.figure(figsize=(12, 12))
plt.imshow(alias_color_mask, interpolation='bilinear')
plt.axis('off')
...
image_colors = ImageColorGenerator(alias_color_mask)
